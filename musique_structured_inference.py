...
if __name__ == "__main__":
    ...
    sampled_ids = list(id2sample.keys())[64:]
    ...
    for i, sample_id in tqdm(enumerate(sampled_ids), total=len(sampled_ids)):
        ...
        df.to_csv(f"musique_gpt4o_mini/final_triplets_{str(sample_id)}.csv")

        df_filtered = pd.DataFrame(faulty_triplets)
        df_filtered['source_text_ids'] = filtered_triplets_source_text_ids
        df_filtered.to_csv(f"musique_gpt4o_mini/faulty_triplets_{str(sample_id)}.csv")
